---
CREATE OR REPLACE FUNCTION mhtc_operations."get_road_casement_section"(road_casement_geom geometry,
...
WHERE ST_INTERSECTS(r.geom, ST_Buffer(c.geom, 0.1))
 ...
 SELECT d.id, ST_Length(mhtc_operations."get_road_casement_section"(d.road_casement_geom, d.corner_geom, 10.0)),
                ST_AsText(mhtc_operations."get_road_casement_section"(d.road_casement_geom, d.corner_geom, 10.0))
 FROM cornerDetails d;


DROP TABLE IF EXISTS mhtc_operations."TestCornerSegments";

CREATE TABLE mhtc_operations."TestCornerSegments"
(
	"id" integer,
...
WHERE ST_INTERSECTS(r.geom, ST_Buffer(c.geom, 0.1))
 ...
 INSERT INTO mhtc_operations."TestCornerSegments" (id, "SegmentLength", geom)
 SELECT d.id, ST_Length(mhtc_operations."get_road_casement_section"(d.road_casement_geom, d.corner_geom, 10.0)),
                ST_AsText(mhtc_operations."get_road_casement_section"(d.road_casement_geom, d.corner_geom, 10.0))
 FROM cornerDetails d;

 ALTER TABLE ONLY mhtc_operations."TestCornerSegments"
    ADD CONSTRAINT "TestCornerSegments_pkey" PRIMARY KEY ("id");
